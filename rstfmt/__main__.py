...
def main() -> None:
    ...
    parser.add_argument('-w', '--width', type=int, default=72, help='the target line length in characters')
    parser.add_argument(
        '--ext',
        default='rst',
        help='the extension of files to look at when passed a directory (default `rst`)',
    )
    parser.add_argument('--test', action='store_true', help='[internal] run tests instead of updating files')
    parser.add_argument('-v', '--verbose', action='store_true', help='[internal] print extra debugging information')
